...
def city_needs_fuel(citytile):
    city = player.cities[citytile.cityid]
    return city.get_light_upkeep()*10 < city.fuel
...
def is_buildable(cell):
    """
    Return True is the cell is not a city and not a resource
    """
    return (not cell.has_resource()) and cell.citytile == None
...
def get_neighborhood_cluster(cluster_cell):
    """
    Returns the list of cells around a cluster (cluster's enveloppe)
    """
    res=[]
    dict_tmp = {}
    cluster = floodfill(cluster_cell.pos)
    if cluster != []:
        for c in cluster:
            neighbors = adjacent_tiles(c.pos)
            for n in neighbors:
                if is_buildable(n):
                    dict_tmp[n] = cluster_cell.pos.distance_to(n.pos)
    
    # Sort cells by distance
    dict_tmp = dict(sorted(dict_tmp.items(), key=lambda item: item[1]))
    res = list(dict_tmp.keys())
    return res
    
    
def get_list_citytiles_in_neighborhood_cluster(cluster_cell):
    res=[]
    dict_tmp = {}
    neighborhood_cluster = get_neighborhood_cluster(cluster_cell)
    for n in neighborhood_cluster:
        if n.citytile != None:
            dict_tmp[n] = cluster_cell.pos.distance_to(n.pos)
...
def get_closest_citytile_in_neighborhood_cluster_needing_fuel(cluster_cell):
    res = None
    dict_tmp = {}
    list_citytiles_in_neighborhood_cluster = get_list_citytiles_in_neighborhood_cluster(cluster_cell)
    for c in list_citytiles_in_neighborhood_cluster:
        if city_needs_fuel(c):
            dict_tmp[c] = cluster_cell.pos.distance_to(c.pos)
        
    dict_tmp = dict(sorted(dict_tmp.items(), key=lambda item: item[1]))
    dict_tmp = list(dict_tmp.keys())
    if dict_tmp != []:
        res = dict_tmp[0]
    return res
    ...
    for i, unit in enumerate(player.units):
        ...
        if unit.is_worker() and unit.can_act():
            if player.city_tile_count==0:
                ...
                #cluster = get_clusters_of_resources(closest_resource_tile)
                if closest_resource_tile != None:
                    neighborhood_cluster = get_neighborhood_cluster(closest_resource_tile)
                    ...
                    # If the cargo is full.
                        ...
                        # If there is no city in the cluster's neighborhood, build one
                        list_citytiles_in_neighborhood_cluster = get_list_citytiles_in_neighborhood_cluster(closest_resource_tile)

                        if list_citytiles_in_neighborhood_cluster==[]:

                            # Build around the cluster
                            if neighborhood_cluster != []:
                                closest_buildable_tile = neighborhood_cluster[0]

                                if unit.pos.equals(closest_buildable_tile.pos):
                                        #print('==DEBUG== Worker on tile_to_build')
                                        if unit.can_build(game_state.map):
                                            #print('==DEBUG== Worker can build')
                                            action = unit.build_city()
                                    ...
                                    move_worker(unit, closest_buildable_tile, positions_occupied_next_round, actions, info_map)
                            else:
                                if unit.pos.equals(closest_city_tile.pos):
                                        #print('==DEBUG== Worker on tile_to_build')
                                        if unit.can_build(game_state.map):
                                            #print('==DEBUG== Worker can build')
                                            action = unit.build_city()
                                    ...
                                    move_worker(unit, closest_city_tile, positions_occupied_next_round, actions, info_map)


                        else:
                            # If there are cities around the cluster, give the resource to the cities if needed, build new one else
                            closest_citytile_in_neighborhood_cluster_needing_fuel = get_closest_citytile_in_neighborhood_cluster_needing_fuel(closest_resource_tile)

                            # If no city needs fuel, build around the cluster
                            if closest_citytile_in_neighborhood_cluster_needing_fuel == []:
                                # Build around the cluster
                                if neighborhood_cluster != []:
                                    closest_buildable_tile = neighborhood_cluster[0]
                            ...
                            # If a city needs fuel, bring resource to it
                            else:
                                closest_city_needing_fuel = closest_citytile_in_neighborhood_cluster_needing_fuel[0]
                                move_worker(unit, closest_city_needing_fuel, positions_occupied_next_round, actions, info_map)
